...
@Component({
  ...
  standalone: true,
  imports: [HeaderComponent,LigasPopularesHandballComponent]
})
export class WidgetsHandballComponent implements AfterViewInit {
  leagueId: string = '';
  season: string = '';
  ...
  initializeWidget(): void {
    const widgetContainer = document.getElementById('wg-api-handball-standings');
    if (widgetContainer) {
      widgetContainer.setAttribute('data-host', 'v1.handball.api-sports.io');
      ...
      widgetContainer.setAttribute('data-team', 'false');
