...
@Component({
  ...
  standalone: true,
  imports: [HeaderComponent,LigasPopularesRugbyComponent]
})
export class WidgetsRugbyComponent implements AfterViewInit {
  leagueId: string = '';
  season: string = '';
  ...
  initializeWidget(): void {
    const widgetContainer = document.getElementById('wg-api-rugby-standings');
    if (widgetContainer) {
      widgetContainer.setAttribute('data-host', 'v1.rugby.api-sports.io');
