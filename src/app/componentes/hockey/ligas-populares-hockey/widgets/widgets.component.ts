...
@Component({
  ...
  standalone: true,
  imports: [HeaderComponent,LigasPopularesHockeyComponent]
})
export class WidgetsHockeyComponent implements AfterViewInit {
  leagueId: string = '';
  season: string = '';
  ...
  initializeWidget(): void {
    const widgetContainer = document.getElementById('wg-api-hockey-standings');
    if (widgetContainer) {
      widgetContainer.setAttribute('data-host', 'v1.hockey.api-sports.io');
